...
function isSelfReference(normalizedText = "", context = {}) {
  ...
  return Boolean(
    requesterName &&
      ...
      new RegExp(`\\b${requesterName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`).test(
        normalizedText
      )
  );
}
...
function extractRegionScheduleFollowupContext(context = {}) {
  ...
  for (const text of texts) {
    const normalized = normalizeText(text);
    const header = text.match(/F3 Wichita Schedule\s+[—-]\s+(.+?)(?:\*|\n|$)/i);
    if (header) {
  ...
  if (events.length === 0) {
    const title =
      intent.type === "scheduled_workouts_by_region"
        ? `F3 Wichita Schedule — ${intent.range.label}`
    ...
    text:
      ...
      rowsToBullets(
        ...
        (event) => {
          const eventLabel =
            intent.type === "scheduled_workouts_by_region"
              ? `${event.aoName || "(unknown AO)"}: ${event.name || "(unnamed workout)"}`
              : event.name || "(unnamed workout)";
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "scheduled_workouts_by_region") {
    ...
    return {
      text: `📅 *F3 Wichita Schedule — ${intent.range.label}*\n\n` + lines.slice(0, 30).join("\n"),
      source: "reporting_db_region",
    };
  }

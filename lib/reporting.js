...
function localRegionScheduleTitle(range) {
  return `${LOCAL_REGION_NAME} Schedule — ${range.label}`;
}
...
function asksRegionWorkoutSchedule(normalized = "") {
  const asksForSchedule =
    /\b(schedule|calendar|workouts?|beatdowns?|events?)\b/.test(normalized) &&
  ...
  const asksForQOnly =
    ...
    !/\bworkouts?|beatdowns?|events?\b/.test(normalized);
...
function asksRegionScheduledQ(normalized = "") {
  ...
  const hasRegionScope =
    /\b(all\s+aos?|every\s+ao|aos?|region|wichita|all\s+workouts?|all\s+beatdowns?|open\s+q\s+slots?)\b/.test(
      normalized
    );
  const hasRange =
...
function asksHardestBeatdowns(normalized = "") {
  ...
  const asksWorkout =
    /\b(beatdowns?|workouts?|bd|bds|q|qs|qing|led|lead|leader|who|which)\b/.test(normalized);
...
function classifySelfPaxReport(text = "", normalized = "") {
  ...
  const postTerms =
    /\b(post|posted|posts|workout|worked out|workouts|beat\s+downs?|beatdown|beatdowns|attended|attendance)\b/.test(
      normalized
    );
  ...
  if (qTerms) {
    if (countTerms) return { type: "self_q_count", range };
    if (aoTerms) return { type: "self_q_aos" };
    if (firstTerms) return { type: "self_q_events", order: "asc", limit: 1, label: "First Q", range };
    if (lastTerms) return { type: "self_q_events", order: "desc", limit: 1, label: "Most Recent Q", range };
    if (historyTerms) return { type: "self_q_events", order: "desc", limit: 10, label: "Q History", range };
  }

  if (postTerms) {
    if (countTerms) return { type: "self_post_count", range };
    if (firstTerms) return { type: "self_post_events", order: "asc", limit: 1, label: "First Post", range };
    if (lastTerms) return { type: "self_post_events", order: "desc", limit: 1, label: "Most Recent Post", range };
    if (historyTerms && /\b(my|mine)\b/.test(normalized)) {
      return { type: "self_post_events", order: "desc", limit: 10, label: "Post History", range };
    }
  }
...
function isBlockedPersonAttendanceQuestion(text = "") {
  ...
  const asksRoster =
    ...
    (/\bwho was\b/.test(normalized) &&
      /\b(at|there|present|attendance|attended|workout|beatdown|ao)\b/.test(normalized));
...
function isThirdPartyAttendanceRequest(text = "", context = {}) {
  ...
  const asksRecentAttendance =
    ...
    /\b(work out|worked out|workouts?|beat\s+downs?|beatdowns?|attended|attendance|posted|posts?)\b/.test(
      normalized
    );
  ...
  const namesOtherPerson =
    !selfReference &&
    (/\blast\b.*\b(workouts?|beat\s+downs?|beatdowns?|posts?)\b.*\bthat\b.*\b(was|were|attended|posted)\b/.test(
      normalized
    ) ||
...
function isUnsupportedReportingStatsQuestion(text = "") {
  ...
  const f3Terms =
    /\b(ao|aos|workout|workouts|beat\s+downs?|beatdown|beatdowns|pax|fng|fngs|q|qs|post|posts|attendance|attend|region|kotter|kotters|wichita)\b/.test(
      normalized
...
function isLikelyReportingDbQuestion(db, text = "") {
  ...
  const eventTerms =
    /\b(q|qs|qed|qd|qing|q ing|pax|attendance|attended|post|posted|posts|workout|workouts|beat\s+downs?|beatdown|beatdowns|backblast|backblasts|boy\s*band|boyband|photo|picture|ao|aos|fng|fngs)\b/.test(
      normalized
    );
...
function isEventDetailFollowUp(text = "") {
  ...
  return (
    ...
    /\b(that|it|this)\b/.test(normalized) &&
    /\b(beatdown|backblast|workout|event|thang|thing)\b/.test(normalized)
  );
}
...
function extractRegionScheduleFollowupContext(context = {}) {
  ...
  for (const text of texts) {
    ...
    if (header) {
      return { range: scheduleRangeFromText(header[1]) };
    }

    if (asksRegionWorkoutSchedule(normalized) && !/\b(q|qs|qing|q ing)\b/.test(normalized)) {
      return { range: scheduleRangeFromText(text) };
    }
  }
...
function classifyReportRequest(text = "", db, context = {}) {
  ...
  if (
    ...
    /\b(where|show|list|was|were|did|attended|attendance|posted|posts?|worked out|workouts?|beat\s+downs?|beatdowns?)\b/.test(
      normalized
    )
  ) {
  ...
  if (
    ...
    /\b(work out|worked out|workouts?|beat\s+downs?|beatdowns?|attended|attendance|posted|posts?)\b/.test(normalized)
  ) {
  ...
  if (
    ...
    /\b(i|me|my|mine)\b/.test(normalized) &&
    /\b(worked out|workout|beat\s+down|beatdown|attended|posted)\b/.test(normalized)
  ) {
  ...
  if (events.length === 0) {
    const title =
      intent.type === "scheduled_workouts_by_region"
        ? localRegionScheduleTitle(intent.range)
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "self_q_count") {
    const resolved = resolveSelfPaxOrReply(db, context);
    if (resolved.error) return resolved.error;
    const rangeClause = intent.range ? "AND start_date BETWEEN ? AND ?" : "";

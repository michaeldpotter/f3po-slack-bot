...
function asksScheduleRangeFollowup(normalized = "") {
  return (
    /\b(show|list|pull|give|get|display|what|whats|what s|can you|could you)\b/.test(normalized) &&
    /\b(next|this|current|upcoming|tomorrow|today|week|month|days?|sun|sunday|mon|monday|tue|tuesday|wed|wednesday|thu|thursday|fri|friday|sat|saturday|satudary)\b/.test(
      normalized
    )
  );
}
...
function asksToFilterPreviousSchedule(normalized = "") {
  return /\b(just|only|filter|narrow|for|at|show|list|pull|get)\b/.test(normalized);
}
          ...
          const eventLabel =
            intent.type === "scheduled_workouts_by_region"
              ? aoName && aoName.localeCompare(eventName, undefined, { sensitivity: "accent" }) !== 0
                ...
                : eventName
              : eventName;
          return `• *${event.startDate}* ${formatEventTime(event.startTime, event.endTime)} — ${eventLabel}: ${formatApiQLabels(event)}`;
        },
        rowLimit
      ),
    source: "f3nation_api",
    ...
    const lines = rows.map((row) => {
      ...
      const qText =
        labels.qs.length > 0 || labels.coqs.length > 0
          ? `; ${formatQLabels(labels).replace(/\n/g, "; ")}`
          : "";
      return `• *${row.start_date}* ${formatEventTime(row.start_time, row.end_time)} — ${row.ao}: ${row.name}${qText}`;
    });

    return {
      text:
        `📅 *${localRegionScheduleTitle(intent.range)}*\n\n` +
        rowsToBulletsWithLimitNote(lines, (line) => line, 100),
      source: "reporting_db_region",
    };
  }

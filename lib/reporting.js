  ...
  if (events.length === 0) {
    return {
      text:
        ...
        `I don’t see upcoming F3 Nation calendar events for ${intent.ao} during ${intent.range.label}.`,
      source: "f3nation_api",
    };
  }
  ...
  return {
    text:
      ...
      rowsToBullets(
        events,
        (event) =>
          `• *${event.startDate}* ${formatEventTime(event.startTime, event.endTime)} — ${event.name || "(unnamed workout)"}: ${formatApiQLabels(event)}`,
        20
      ),
    source: "f3nation_api",
  };

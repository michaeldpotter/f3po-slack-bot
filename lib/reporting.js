...
// Keep common PAX phrasing together so "beatdown"/"BD", "boot camp"/"BC",
...
const WORKOUT_TERM_RE = new RegExp(
  `\\b(bds?|bcs?|work\\s*out|worked\\s*out|workouts?|beat\\s*downs?|boot\\s*camps?|bootcamps?|${RUCK_TERM_PATTERN})\\b`
);
...
function currentMonthRange() {
  ...
  return { start: isoDate(start), end: currentDate(), label: "this month" };
}
...
function dateRangeFromText(text = "") {
  ...
  if (/\blast\s+7\s+days\b/.test(normalized)) {
    return { start: isoDateDaysAgo(7), end: currentDate(), label: "last 7 days" };
  }
  if (/\blast\s+week\b/.test(normalized)) {
    return { start: isoDateDaysAgo(7), end: currentDate(), label: "last week" };
  }
...
function explicitReportingRangeFromText(text = "") {
  const normalized = normalizeText(text);
  return (
    monthRangeFromText(text) ||
    explicitYearRangeFromText(text) ||
    (/\b(this year|this month|last week|last 7 days|last month|last 30 days|last 90 days|last year|last 12 months)\b/.test(
      normalized
    )
      ? dateRangeFromText(text)
      : null)
  );
}
...
function asksFavoriteAo(normalized = "") {
  return (
    /\b(ao|aos|site|location|place|spot)\b/.test(normalized) &&
    /\b(fav|favorite|favourite|home|go\s*to|goto|main|usual|regular|most\s+visited|most\s+posted)\b/.test(normalized)
  );
}
...
function classifySelfPaxReport(text = "", normalized = "") {
  ...
  const qTerms = /\b(q|qs|qed|q ed|qd|q d|qing|q ing|led|lead|leading)\b/.test(normalized);
...
function classifyNamedPaxPostReport(db, text = "", normalized = "") {
  const selfScoped = /\b(i|my|mine|myself)\b/.test(normalized) || /\b(of|for|with)\s+me\b/.test(normalized);
  if (selfScoped) return null;
  if (/\b(together|with)\b/.test(normalized)) return null;
  ...
  const countTerms = /\b(how many|count|counts|number|total|times)\b/.test(normalized);
  const aoTerms = /\b(where|which ao|what ao|aos|places|locations)\b/.test(normalized);
  const broadRecentList = /\b(last|latest|recent|most recent)\b/.test(normalized) && /\b(2|3|4|5|6|7|8|9|10|two|three|four|five|six|seven|eight|nine|ten)\b/.test(normalized);
  if (!postTerms || broadRecentList) return null;

  const range = explicitReportingRangeFromText(text);
  const postNoun = workoutKind === "ruck" ? "Ruck" : "Post";
  ...
  if (!firstTerms && !lastTerms) return null;

  return {
    type: "pax_post_events",
    paxName,
    order: firstTerms ? "asc" : "desc",
    limit: 1,
    label: `${firstTerms ? "First" : "Most Recent"} ${postNoun}`,
    range,
    workoutKind,
  };
}
...
function isBlockedPersonAttendanceQuestion(text = "") {
  ...
  const selfQuestion = /\b(i|my|mine|myself)\b/.test(normalized) || /\b(of|for|with)\s+me\b/.test(normalized);
  ...
  const boundedHistoricalQuestion = /\b(first|oldest|earliest)\b/.test(normalized);

  if (asksRoster) return true;
  return asksAttendance && !selfQuestion && !aggregateQuestion && !boundedHistoricalQuestion;
}
...
function isThirdPartyAttendanceRequest(text = "", context = {}) {
  ...
  const namesOtherPerson =
    !selfReference &&
    (/\blast\b.*\b(bds?|bcs?|work\s*out|worked\s*out|workouts?|beat\s*downs?|boot\s*camps?|bootcamps?|rucks?|rucking|hikes?|hiking|posts?)\b.*\bthat\b.*\b(was|were|attended|posted)\b/.test(
      normalized
    ) ||
...
function isLikelyReportingDbQuestion(db, text = "") {
  ...
  const eventTerms =
    /\b(q|qs|qed|q ed|qd|q d|qing|q ing|pax|attendance|attended|post|posted|posts|backblast|backblasts|boy\s*band|boyband|photo|picture|ao|aos|fng|fngs)\b/.test(
      normalized
    ) || hasWorkoutTerm(normalized);
...
function isEventQFollowUp(text = "") {
  ...
  return (
    ...
    /\b(q\s+(ed|d)|qed|q ed|qd|q d|q|qs)\b/.test(normalized) &&
    (/\b(that|it|this)\b/.test(normalized) || /\b(was|were|is|are|had|has)\b/.test(normalized))
  );
}

function isEventRosterFollowUp(text = "") {
  const normalized = normalizeText(text);
  return (
    /\b(who|which pax|pax|roster|attendance)\b/.test(normalized) &&
    /\b(there|attended|posted|present|at|roster|attendance|with\s+(him|her|them|that|it))\b/.test(normalized)
  );
}
...
function extractReferencedEventFromText(text = "") {
  ...
  if (detailMatch) {
    return {
      name: normalizeEventTitle(detailMatch[1]),
      ao: normalizeEventTitle(detailMatch[2]),
      startDate: detailMatch[3],
    };
  }
...
function reportingContextFromMessage(message = {}) {
  return (
...
function extractReferencedEventFromContext(context = {}) {
  ...
  for (const message of orderedMessages) {
    const reportingContext = reportingContextFromMessage(message);
    if (reportingContext?.event?.startDate && reportingContext.event?.ao && reportingContext.event?.name) {
      return reportingContext.event;
    }
  }

  for (const message of orderedMessages) {
    const event = extractReferencedEventFromText(message?.text || "");
...
function extractPaxReportFollowupContext(context = {}) {
  const messages = Array.isArray(context.threadMessages) ? context.threadMessages : [];
  const orderedMessages = [...messages].reverse();

  for (const message of orderedMessages) {
    const reportingContext = reportingContextFromMessage(message);
    if (["self_favorite_ao", "pax_favorite_ao"].includes(reportingContext?.type)) {
      return { type: "pax_favorite_ao", workoutKind: reportingContext.workoutKind || "" };
    }

    if (["pax_post_events", "pax_q_events"].includes(reportingContext?.type)) {
      return {
        type: reportingContext.type,
        order: reportingContext.order,
        limit: reportingContext.limit || 1,
        label: reportingContext.label,
        range: reportingContext.range || null,
        workoutKind: reportingContext.workoutKind || "",
      };
    }
  }
  ...
  for (const message of orderedMessages) {
    ...
    const title = text.match(/\*\s*.+?'s\s+(.+?)(?:\s+[—-]\s+.+?)?\s*\*/);
    if (!title) continue;

    const label = title[1].trim();
    const normalizedLabel = normalizeText(label);
    if (asksFavoriteAo(normalizedLabel)) {
      return { type: "pax_favorite_ao" };
    }

    const first = /\bfirst\b/.test(normalizedLabel);
    const recent = /\b(most recent|last|latest)\b/.test(normalizedLabel);
    if (!first && !recent) continue;

    const workoutKind = /\bruck\b/.test(normalizedLabel) ? "ruck" : "";
    if (/\bq\b/.test(normalizedLabel)) {
      return {
        type: "pax_q_events",
        order: first ? "asc" : "desc",
        limit: 1,
        label: first ? "First Q" : "Most Recent Q",
      };
    }

    if (/\b(post|beatdown|bd|bc|bootcamp|boot camp|workout|ruck)\b/.test(normalizedLabel)) {
      return {
        type: "pax_post_events",
        order: first ? "asc" : "desc",
        limit: 1,
        label: `${first ? "First" : "Most Recent"} ${workoutKind === "ruck" ? "Ruck" : "Post"}`,
        workoutKind,
      };
    }
  }
...
function classifyReportRequest(text = "", db, context = {}) {
  ...
  const namedPaxInRequest = matchPaxName(db, text);
  const requesterPax = context.requesterName ? matchSlackNameToPax(db, context.requesterName)?.name || "" : "";
  const namedPaxIsRequester =
    namedPaxInRequest &&
    requesterPax &&
    normalizeText(namedPaxInRequest.replace(/\([^)]*\)/g, " ")) ===
      normalizeText(requesterPax.replace(/\([^)]*\)/g, " "));
  const commandMeOnly = namedPaxInRequest && !namedPaxIsRequester && !/\b(i|my|mine|myself)\b/.test(normalized);
  const selfQuestion = isSelfReference(normalized, context) && !commandMeOnly;
  ...
  if (selfQuestion) {
    if (asksFavoriteAo(normalized)) {
      return { type: "self_favorite_ao", workoutKind: requestedWorkoutKind(normalized) };
    }
  ...
  if (asksFavoriteAo(normalized)) {
    const paxName = matchPaxName(db, text);
    if (paxName) return { type: "pax_favorite_ao", paxName, workoutKind: requestedWorkoutKind(normalized) };
  }

  const namedPaxPostReport = classifyNamedPaxPostReport(db, text, normalized);
  if (namedPaxPostReport) return namedPaxPostReport;

  if (/\b(what about|how about|same for|for)\b/.test(normalized)) {
    const paxName = matchPaxName(db, text);
    const previousPaxReport = paxName ? extractPaxReportFollowupContext(context) : null;
    if (paxName && previousPaxReport) {
      return {
        ...previousPaxReport,
        paxName,
      };
  ...
  if (isEventRosterFollowUp(text)) {
    const event = extractReferencedEventFromContext(context);
    if (event) {
      return { type: "event_roster_followup", event };
    }
  }
  ...
  if (
    ...
    /\b(q|qs|qed|q ed|qd|q d)\b/.test(normalized) &&
    ...
    return { type: "q_leaderboard_overall", range };
  }

  if (
    /\b(who|leader|leaderboard|most|top)\b/.test(normalized) &&
    (hasWorkoutOrAttendanceTerm(normalized) || hasWorkoutTerm(normalized) || /\bpax\b/.test(normalized)) &&
    /\b(most|top|leader|leaderboard)\b/.test(normalized)
  ) {
    return {
      type: "post_leaderboard_overall",
      range: monthRangeFromText(text) || dateRangeFromText(text),
      workoutKind: requestedWorkoutKind(normalized),
    };
  }
...
function favoriteAoReport(db, paxName, source, workoutKind = "") {
  const kindClause = workoutKindSqlClause(workoutKind, "e");
  const reportNoun = workoutKind === "ruck" ? "Rucking AO" : "AO";
  const countNoun = workoutKind === "ruck" ? "ruck" : "post";
  const rows = db
    .prepare(
      `SELECT COALESCE(NULLIF(e.ao_name, ''), '(unknown)') AS ao,
       ...
       WHERE a.f3_name = ? COLLATE NOCASE
         ${kindClause}
       GROUP BY ao
       ORDER BY post_count DESC, ao COLLATE NOCASE ASC
       LIMIT 5`
    )
    .all(paxName);
  ...
  if (rows.length === 0) {
    return {
      text: `I matched ${paxName}, but I don’t see matching ${countNoun} attendance rows in the local reporting DB.`,
      source,
    };
  }
  ...
  return {
    text:
      `📍 *${paxName}'s Favorite ${reportNoun}*\n\n` +
      `Using most ${countNoun}s as the signal, it looks like *${top.ao}* (${top.post_count} ${countNoun}${top.post_count === 1 ? "" : "s"}, last ${top.last_post}).\n\n` +
      `*Top ${reportNoun}s*\n${numberedRows(rows, (row) => `${row.ao} — ${row.post_count}`, 5)}`,
    source,
  };
}
...
function findReferencedEventRows(db, event) {
  const eventRows = db
    .prepare(
      `SELECT id,
              start_date,
    ...
    .all(event.startDate, event.ao, event.name);
  ...
  return eventRows.length > 0
    ...
    : db
        ...
        .all(event.startDate, event.ao);
}

function getEventRoster(db, eventId) {
  return db
    .prepare(
      `SELECT f3_name,
              q_ind,
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "self_favorite_ao") {
    const resolved = resolveSelfPaxOrReply(db, context);
    if (resolved.error) return resolved.error;
    return favoriteAoReport(db, resolved.name, "reporting_db_self", intent.workoutKind || "");
  }

  if (intent.type === "pax_favorite_ao") {
    return favoriteAoReport(db, intent.paxName, "reporting_db_pax", intent.workoutKind || "");
  }
  ...
  if (intent.type === "pax_post_events") {
    const direction = intent.order === "asc" ? "ASC" : "DESC";
    const rangeClause = intent.range ? "AND e.start_date BETWEEN ? AND ?" : "";
    const kindClause = workoutKindSqlClause(intent.workoutKind || "", "e");
    const params = intent.range
      ? [intent.paxName, intent.range.start, intent.range.end, intent.limit]
      : [intent.paxName, intent.limit];
    const rows = db
      .prepare(
        `SELECT e.start_date,
                e.start_time,
    ...
    if (rows.length === 0) {
      return {
        text: `I matched ${intent.paxName}, but I don’t see matching attendance rows in the local reporting DB.`,
        source: "reporting_db_pax",
      };
    }
    ...
    return {
      text:
        `🧾 *${intent.paxName}'s ${intent.label}${intent.range ? ` — ${intent.range.label}` : ""}*\n\n` +
        rowsToBullets(
          rows,
          (row) => {
            const role = row.q_ind ? ", Q" : row.coq_ind ? ", Co-Q" : "";
  ...
  if (intent.type === "pax_post_count") {
    const rangeClause = intent.range ? "AND e.start_date BETWEEN ? AND ?" : "";
    const kindClause = workoutKindSqlClause(intent.workoutKind || "", "e");
    const params = intent.range ? [intent.paxName, intent.range.start, intent.range.end] : [intent.paxName];
    const row = db
      .prepare(
      ...
      .get(...params);
    const label = intent.workoutKind === "ruck" ? "Ruck Count" : "Post Count";
    const noun = intent.workoutKind === "ruck" ? "Rucks" : "Posts";
    return {
      text:
        `🧾 *${intent.paxName}'s ${label}${intent.range ? ` — ${intent.range.label}` : ""}*\n\n` +
        `• ${noun}: *${row?.post_count || 0}*\n` +
        `• AOs: ${row?.ao_count || 0}\n` +
        `• First ${intent.workoutKind === "ruck" ? "ruck" : "post"}: ${row?.first_post || "n/a"}\n` +
        `• Most recent ${intent.workoutKind === "ruck" ? "ruck" : "post"}: ${row?.last_post || "n/a"}`,
      source: "reporting_db_pax",
    };
  }

  if (intent.type === "pax_post_aos") {
    const rangeClause = intent.range ? "AND e.start_date BETWEEN ? AND ?" : "";
    const kindClause = workoutKindSqlClause(intent.workoutKind || "", "e");
    const params = intent.range ? [intent.paxName, intent.range.start, intent.range.end] : [intent.paxName];
    const rows = db
    ...
    return {
      text:
        `📍 *AOs ${intent.paxName} Has Posted${intent.range ? ` — ${intent.range.label}` : ""}*\n\n` +
        rowsToBullets(rows, (row) => `• *${row.ao}:* ${row.post_count} post(s), last ${row.last_post}`, 20),
      source: "reporting_db_pax",
    };
  }
  ...
  if (intent.type === "q_leaderboard_overall") {
    const rows = db
      .prepare(
        `SELECT a.f3_name,
                COUNT(*) AS q_count
    ...
    if (rows.length === 0) {
      return {
        text: `I don’t have any marked Qs during ${intent.range.label}.`,
        source: "reporting_db",
      };
    }
    ...
    return {
      text:
        `🏆 *Q Leaderboard — ${intent.range.label}*\n\n` +
        `*Leader${leaders.length === 1 ? "" : "s"}:* ${leaders.join(", ")} (${topCount} Q${topCount === 1 ? "" : "s"})\n\n` +
        `*Top Qs*\n` +
        numberedRows(rows, (row) => `${row.f3_name} — ${row.q_count}`, 10),
      source: "reporting_db",
  ...
  if (intent.type === "post_leaderboard_overall") {
    const kindClause = workoutKindSqlClause(intent.workoutKind || "", "e");
    const rows = db
      .prepare(
        `SELECT a.f3_name,
         ...
         WHERE e.start_date BETWEEN ? AND ?
           ...
           AND a.f3_name != ''
         GROUP BY a.f3_name
         ORDER BY post_count DESC, a.f3_name COLLATE NOCASE ASC
         LIMIT 10`
      )
      .all(intent.range.start, intent.range.end);

    const noun = intent.workoutKind === "ruck" ? "Ruck" : "Post";
    if (rows.length === 0) {
      return {
        text: `I don’t have any ${noun.toLowerCase()} rows during ${intent.range.label}.`,
        source: "reporting_db",
      };
    }

    const topCount = rows[0].post_count;
    const leaders = rows.filter((row) => row.post_count === topCount).map((row) => row.f3_name);
    return {
      text:
        `🏆 *${noun} Leaderboard — ${intent.range.label}*\n\n` +
        `*Leader${leaders.length === 1 ? "" : "s"}:* ${leaders.join(", ")} (${topCount} ${noun}${topCount === 1 ? "" : "s"})\n\n` +
        `*Top ${noun}s*\n` +
        numberedRows(rows, (row) => `${row.f3_name} — ${row.post_count}`, 10),
      source: "reporting_db",
    };
  }
  ...
  if (intent.type === "event_q_followup") {
    const fallbackRows = findReferencedEventRows(db, intent.event);
  ...
  if (intent.type === "event_roster_followup") {
    const fallbackRows = findReferencedEventRows(db, intent.event);
    ...
    const event = fallbackRows[0];
    const roster = getEventRoster(db, event.id);

    if (roster.length === 0) {
      return {
        text: `I found ${event.name} at ${event.ao} on ${event.start_date}, but the local reporting DB doesn’t have an attendance roster for that event.`,
        source: "reporting_db",
      };
    }
...
function buildReportingContext(intent = {}, report = {}) {
  if (!intent || !report) return null;
  const event = intent.event || extractReferencedEventFromText(report.text || "");

  if (intent.type === "self_post_events" || intent.type === "pax_post_events") {
    return {
      type: "pax_post_events",
      order: intent.order,
      limit: intent.limit || 1,
      label: intent.label,
  ...
  if (intent.type === "self_q_events" || intent.type === "pax_q_events") {
    return {
      type: "pax_q_events",
      order: intent.order,
      limit: intent.limit || 1,
  ...
  if (intent.type === "self_favorite_ao" || intent.type === "pax_favorite_ao") {
    return { type: "pax_favorite_ao", workoutKind: intent.workoutKind || "" };
  }

  if (["event_q_followup", "event_roster_followup", "event_detail_followup"].includes(intent.type)) {
    return {
      type: intent.type,
      event,
    };
  ...
  return event ? { type: intent.type, event } : { type: intent.type };
}
...
async function maybeAnswerReportingQuestion(text, options = {}) {
  ...
  try {
    ...
    const report = await runReportWithLiveSources(db, intent, context);
    if (!report) return report;
    return {
      ...report,
      reportingContext: buildReportingContext(intent, report),
    };
  } finally {
    db.close();
  }

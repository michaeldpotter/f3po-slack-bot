...
function extractScheduleFollowupContext(db, context = {}) {
  ...
  for (const text of texts) {
    if (!/\b(qing|scheduled to q|who is q|who's q|leading)\b/i.test(text)) continue;
...
function classifyReportRequest(text = "", db, context = {}) {
  ...
  if (
    /\b(who|which|show|list|tell)\b/.test(normalized) &&
    /\b(qing|scheduled to q|signed up to q|leading|q spots?|q assignments?)\b/.test(normalized) &&
    ...
    if (ao) {
      return {
        type: "scheduled_q_by_ao",
        ao,
        range: weekRangeFromText(text) || dateRangeFromText(text),
      };
    }
  }
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "scheduled_q_by_ao") {
    ...
    const checkCalendarText =
      "For future Q assignments, check the F3 Nation app with `/calendar`, the #-calendar channel, or that specific AO channel.";

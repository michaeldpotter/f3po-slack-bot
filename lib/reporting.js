...
function classifySelfPaxReport(text = "", normalized = "") {
  ...
  const postTerms =
    /\b(post|posted|posts|workout|worked out|workouts|beatdown|beatdowns|attended|attendance)\b/.test(
      normalized
    );
  ...
  if (qTerms) {
    if (countTerms) return { type: "self_q_count" };
    if (aoTerms) return { type: "self_q_aos" };
    if (firstTerms) return { type: "self_q_events", order: "asc", limit: 1, label: "First Q" };
    if (lastTerms) return { type: "self_q_events", order: "desc", limit: 1, label: "Most Recent Q" };
    if (historyTerms) return { type: "self_q_events", order: "desc", limit: 10, label: "Q History" };
  }

  if (postTerms) {
    if (countTerms) return { type: "self_post_count" };
    if (firstTerms) return { type: "self_post_events", order: "asc", limit: 1, label: "First Post" };
    if (lastTerms) return { type: "self_post_events", order: "desc", limit: 1, label: "Most Recent Post" };
    if (historyTerms && /\b(my|mine)\b/.test(normalized)) {
      return { type: "self_post_events", order: "desc", limit: 10, label: "Post History" };
    }
  }
...
function isThirdPartyAttendanceRequest(text = "", context = {}) {
  ...
  const asksRecentAttendance =
    ...
    /\b(work out|worked out|workouts?|beatdowns?|attended|attendance|posted|posts?)\b/.test(
      normalized
    );
  ...
  const namesOtherPerson =
    !selfReference &&
    (/\blast\b.*\b(workouts?|beatdowns?|posts?)\b.*\bthat\b.*\b(was|were|attended|posted)\b/.test(
      normalized
    ) ||
...
function isLikelyReportingDbQuestion(db, text = "") {
  ...
  const eventTerms =
    /\b(q|qs|qed|qd|qing|q ing|pax|attendance|attended|post|posted|posts|workout|workouts|beatdown|beatdowns|backblast|backblasts|boy\s*band|boyband|photo|picture|ao|aos|fng|fngs)\b/.test(
      normalized
    );
...
function classifyReportRequest(text = "", db, context = {}) {
  ...
  if (
    ...
    /\b(where|show|list|was|were|did|attended|attendance|posted|posts?|worked out|workouts?|beatdowns?)\b/.test(
      normalized
    )
  ) {
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "self_q_events") {
    ...
    const rows = db
      ...
      .all(resolved.name, intent.limit);

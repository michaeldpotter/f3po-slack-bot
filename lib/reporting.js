...
function classifyReportRequest(text = "", db, context = {}) {
  ...
  if (asksFavoriteAo(normalized)) {
    const paxName = matchPaxName(db, text);
    if (paxName) return { type: "pax_favorite_ao", paxName };
  }
...
function favoriteAoReport(db, paxName, source) {
  const rows = db
    .prepare(
  ...
  if (rows.length === 0) {
    return {
      text: `I matched ${paxName}, but I don’t see attendance rows in the local reporting DB.`,
      source,
    };
  }
  ...
  return {
    text:
      `📍 *${paxName}'s Favorite AO*\n\n` +
      `Using most posts as the signal, it looks like *${top.ao}* (${top.post_count} post${top.post_count === 1 ? "" : "s"}, last ${top.last_post}).\n\n` +
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "self_favorite_ao") {
    ...
    return favoriteAoReport(db, resolved.name, "reporting_db_self");
  }

  if (intent.type === "pax_favorite_ao") {
    return favoriteAoReport(db, intent.paxName, "reporting_db_pax");
  }

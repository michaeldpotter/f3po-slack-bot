...
function isBlockedPersonAttendanceQuestion(text = "") {
  ...
  const aggregateQuestion =
    /\b(average|avg|count|counts|total|totals|how many|by month|by ao|all aos|fngs|highest|max|maximum|record|ever)\b/.test(
      normalized
    );
...
function classifyReportRequest(text = "", db, context = {}) {
  ...
  if (
    /\b(highest|max|maximum|record)\b/.test(normalized) &&
    /\b(attendance|pax)\b/.test(normalized)
  ) {
...
function runReport(db, intent, context = {}) {
  ...
  if (intent.type === "event_q_followup") {
    ...
    const qs = qRows.filter((row) => row.q_ind).map((row) => row.f3_name);
    const coqs = qRows.filter((row) => row.coq_ind && !row.q_ind).map((row) => row.f3_name);

    if (qs.length === 0 && coqs.length === 0) {
      return {
    ...
    const qText = qs.length > 0 ? `Q: ${qs.join(", ")}` : "";
    const coqText = coqs.length > 0 ? `Co-Q: ${coqs.join(", ")}` : "";
    return {
      text:
        `${event.name} at ${event.ao} on ${event.start_date} had ${event.pax_count} PAX.\n` +
        [qText, coqText].filter(Boolean).join("\n"),

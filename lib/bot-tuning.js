...
const DEFAULT_REPLY_STYLE = "brief";
const DEFAULT_THREAD_FOLLOW_UP_MODE = "conservative";
const DEFAULT_MESSAGE_DEDUPE_TTL_MS = 5 * 60 * 1000;
const DEFAULT_QUESTION_DEDUPE_TTL_MS = 2 * 60 * 1000;
const DEFAULT_THREAD_REPLY_LIMIT = 4;
const DEFAULT_THREAD_REPLY_LIMIT_WINDOW_MS = 60 * 1000;
const DEFAULT_MAX_THREAD_MESSAGES = 20;
const DEFAULT_MAX_CHARS_PER_MESSAGE = 2000;
const DEFAULT_LOCAL_REGION_NAME = "F3 Wichita";
const DEFAULT_WEB_SEARCH_ALLOWED_DOMAINS = ["f3nation.com"];
...
function normalizeReplyStyle(value = "") {
  ...
  return REPLY_STYLES.has(normalized) ? normalized : DEFAULT_REPLY_STYLE;
}

function normalizeThreadFollowUpMode(value = "") {
  const normalized = value.trim().toLowerCase().replace(/-/g, "_");
  return THREAD_FOLLOW_UP_MODES.has(normalized) ? normalized : DEFAULT_THREAD_FOLLOW_UP_MODE;
}

function loadBotTuning() {
  return {
    ...
    replyStyle: normalizeReplyStyle(DEFAULT_REPLY_STYLE),
    ...
    threadFollowUpMode: normalizeThreadFollowUpMode(DEFAULT_THREAD_FOLLOW_UP_MODE),
    ...
    messageDedupeTtlMs: DEFAULT_MESSAGE_DEDUPE_TTL_MS,
    ...
    questionDedupeTtlMs: DEFAULT_QUESTION_DEDUPE_TTL_MS,
    ...
    threadReplyLimit: DEFAULT_THREAD_REPLY_LIMIT,
    threadReplyLimitWindowMs: DEFAULT_THREAD_REPLY_LIMIT_WINDOW_MS,
    ...
    localRegionName: DEFAULT_LOCAL_REGION_NAME,

    // Limits web fallback to approved domains only.
    // Edit DEFAULT_WEB_SEARCH_ALLOWED_DOMAINS above to change this. Keeping this in
    // code avoids stale .env values quietly widening what F3PO can browse.
    webSearchAllowedDomains: DEFAULT_WEB_SEARCH_ALLOWED_DOMAINS,
  };
}
...
module.exports = {
  ...
  DEFAULT_MESSAGE_DEDUPE_TTL_MS,
  DEFAULT_LOCAL_REGION_NAME,
  DEFAULT_QUESTION_DEDUPE_TTL_MS,
  DEFAULT_REPLY_STYLE,
  DEFAULT_THREAD_FOLLOW_UP_MODE,
  DEFAULT_THREAD_REPLY_LIMIT,
  DEFAULT_THREAD_REPLY_LIMIT_WINDOW_MS,
